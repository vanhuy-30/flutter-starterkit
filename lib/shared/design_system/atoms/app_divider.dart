...
class AppDivider extends StatelessWidget {
        ...
        child: Container(
          ...
          child: LayoutBuilder(
            ...
            return SizedBox(
                height: height,
                child: Row(
                  children: _buildDividerLine(
                      constraints.maxWidth, dividerColor, dividerThickness),
                ));
          }),
        ));
  }

  List<Widget> _buildDividerLine(
      double maxWidth, Color dividerColor, double dividerThickness) {
    if (type == DividerType.normal) {
      return [
        Expanded(
          child: Container(height: dividerThickness, color: dividerColor),
        )
      ];
    } else {
      return [
        Expanded(
            child: Row(
                ...
                children:
                    _buildPattern(maxWidth, dividerColor, dividerThickness)))
      ];
    }
  }
  ...
  List<Widget> _buildPattern(
      double maxWidth, Color dividerColor, double dividerThickness) {
    const patternLength = 4.0;
    const patternSpace = 4.0;
    ...
    for (int i = 0; i < numberOfPattern; i++) {
      dividers.add(Container(
          width: patternLength, height: dividerThickness, color: dividerColor));
    ...
    return dividers as List<Widget>;
  }
}

enum DividerType { normal, dotted, dashed }
